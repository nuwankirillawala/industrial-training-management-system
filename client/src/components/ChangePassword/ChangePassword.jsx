...
export const ChangePassword = () => {
    ...
    const validatePwd = Yup.object().shape({
        currentpassword : Yup.string().length(6,"You must enter 6 characters").required('Enter your current password'),
        newPassword : Yup.string().matches(
                                    /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{6,})/,
                                    "Must Contain 6 Characters, One Uppercase, One Lowercase, One Number and One Special Case Character"
                                  ).required('Enter your new password'),
        confirmPassword : Yup.string().oneOf([Yup.ref("newPassword")], "Your password do not match.").required('Confirm your new password')
    })
                ...
                <form onSubmit={handleSubmit}>
                    <Stack direction={'column'} spacing={1}>
                        ...
                        <TextField
                            ...
                            helperText={touched.newPassword && errors.newPassword}InputProps={{
                                endAdornment: (
                                  <InputAdornment position="end">
                                    <IconButton
                                      aria-label="toggle password visibility"
                        ...
                        <TextField
                            ...
                            helperText={touched.confirmPassword && errors.confirmPassword}InputProps={{
                                endAdornment: (
                                  <InputAdornment position="end">
                                    <IconButton
                                      aria-label="toggle password visibility"
